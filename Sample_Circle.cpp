...
void reshapeWindow (GLFWwindow* window, int width, int height)
{
  ...
  // Store the projection matrix in a variable for future use
    ...
    Matrices.projection = glm::ortho(-4.0f/*(left)*/, 4.0f, -4.0f, 4.0f, 0.1f/*depth*/, 500.0f);
}
...
void createTriangle ()
{
  ...
  for(i=0;i<36;i++){
    ...
    vertex_buffer_data[j]=.1 * cosf(theta);j++;
    vertex_buffer_data[j]=.1* sinf(theta);j++;
    vertex_buffer_data[j]=0;j++;
  ...
  triangle = create3DObject(GL_TRIANGLES, 108, vertex_buffer_data, color_buffer_data, GL_LINE);
}
...
void draw (float i)
{
  ...
  glm::mat4 translateTriangle = glm::translate (glm::vec3(-4.0f+i , (4.0f*i)-(i*i), 0.0f)); // glTranslatef
  //glm::mat4 rotateTriangle = glm::rotate((float)(triangle_rotation*M_PI/180.0f), glm::vec3(0,0,1));  // rotate about vector (1,0,0)
  glm::mat4 triangleTransform = translateTriangle;
  Matrices.model *= triangleTransform;
  MVP = VP * Matrices.model; // MVP = p * V * M
//model view pojection matrix
  //  Don't change unless you are sure!!
